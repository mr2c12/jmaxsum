...
do
	...
	do
		ITER=`grep $C "dsaiter.csv" | cut -f 2 -d ,`
		echo $ITER
		mkdir -p $SUBDIR/$C
		...
		do
			...
			do
				TMP=`mktemp`
				java -jar ../jmaxsum.jar --generators $M --loads $D --ancillary $R --center $C --width $W --seed $SEED --algorithm dsa --dsa $DSA --probability $P --report $TMP --time --iterations $ITER
